...
import { gettingStartedSections } from "../data/getting-started-docs";

export default function GettingStartedPage() {
  const { setActiveSectionId } = useTOCContext();
  const { current, currentIndex, goTo } =
    useSectionNavigator(gettingStartedSections);
  ...
  return (
    <div id="doc-content" className="relative px-4 pb-16">
      ...
      <div className="flex justify-between mt-28">
        ...
        <button
          onClick={() => goTo(currentIndex + 1)}
          disabled={currentIndex === gettingStartedSections.length - 1}
          className={`px-8 py-3 rounded-md text-sm font-medium ${
            currentIndex === gettingStartedSections.length - 1
              ? "bg-gray-200 text-gray-400 cursor-not-allowed"
              : "bg-blue-600 text-white cursor-pointer"
          }`}
        >
